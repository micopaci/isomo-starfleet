...
import type { CreateSiteInput } from '@starfleet/shared';

interface Props {
  onSave: (input: CreateSiteInput) => Promise<void>;
  onClose: () => void;
}

export function SiteCreateModal({ onSave, onClose }: Props) {
  const [form, setForm] = useState({
    name: '', starlink_sn: '', location: '', district: '',
    lat: '', lng: '', kit_id: '', starlink_uuid: '',
  });
    ...
    <div style={overlay} onClick={e => { if (e.target === e.currentTarget) onClose(); }}>
      <div style={dialog}>
        <div style={{ display: 'flex', alignItems: 'center', marginBottom: 16, gap: 8 }}>
          <h2 style={{ flex: 1, margin: 0, fontSize: 16 }}>Add new dish / site</h2>
