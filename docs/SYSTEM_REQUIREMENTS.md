...
| Static web dashboard | Provides Vercel-hosted browser/PWA access and same-origin backend-served local access |
| Desktop app | Provides Electron operator-console access for desktop users |
| Mobile app | Provides React Native Android field/admin companion access |
| Shared client | Provides cross-platform API client, WebSocket client, hooks, and types |
...
### 4.5 Dashboard, Desktop, And Mobile

| ID | Requirement | Priority | Status |
|---|---|---|---|
| FR-040 | Show site inventory, online laptop counts, signal metrics, score, weather predictor, and usage | Must | Implemented |
| FR-041 | Show laptop list with Intune-first online/stale/offline status and health metadata | Must | Implemented |
| FR-042 | Show site detail with connected devices, latest signal, latency, and usage context | Must | Implemented |
| FR-043 | Show historical latency, signal, and monthly usage charts | Should | Implemented/In progress by surface |
| FR-044 | Support mobile field/admin workflows for overview, map, campuses, Starlinks, alerts, and settings | Should | In progress |
| FR-045 | Support admin-triggered Intune remediation actions for device and site scopes | Should | Implemented, requires Graph policy IDs |
| FR-046 | Support monthly Starlink portal usage import and CSV exports | Should | Implemented |
...
| Client | Requirement |
|---|---|
| Web dashboard | Modern browser, HTTPS production origin, backend CORS origin allowed |
| Desktop dashboard | macOS/Windows/Linux Electron runtime generated by Electron Builder |
| Mobile app | Android package `com.isomo.starfleet`; API base configurable in Settings |
| Shared client | Dashboard and mobile clients must initialize `StarfleetApi` and `StarfleetWS` before calling shared hooks |
...
| Intune devices | Intune device ID, Azure AD device ID, enrolled/sync timestamps, compliance, user principal, category, storage |
...
1. Build with `npm run build --workspace=packages/desktop`.
2. Install/open the generated Electron package.
3. Confirm API base URL, login, WebSocket updates, overview, Starlinks, computers, alerts, map, and site detail.

### 8.4 Mobile

1. Build/install the Android app from `packages/mobile`.
2. Set the API base URL in Settings when the default is not production.
3. Verify login, overview, map, campuses, Starlinks, alerts, site detail, and sign out.
...
| Mobile release channel | Admin/Ops | Decide whether Android builds are internal APK, managed Play, or MDM-distributed |
| Desktop release channel | Admin/Ops | Decide how Electron builds are signed and distributed |
